    ...
    return (
      <div className="menu-item" data-id={id}>
        <div className="menu-item-info">
          ...
          <div className="menu-item-name">{duration_hrs}</div>
        </div>
...
export default function Page() {
  ...
  return (
    <div>
      <button className="add-button" onClick={() => router.push(`userevents/addevents`)}>
        Add
      </button>
      {displaySuccessMessage.show && (
      ...
      {eventItems ? (
        eventItems.map((item) => (
          <Eventbox
            ...
            onEdit={() => router.push(`userevents/edit/${item.id}`)}
